...
export default function CreateWorkshopPage() {
  ...
  const [formData, setFormData] = useState<{
    ...
    imageUrl: "",
    videoUrl: "",
    description: "**Write your workshop description here...**",
    price: 0,
    originalPrice: 0,
    currency: "EGP",
    durationHours: 0,
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                ...
                name="slug"
                required
                className="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50 text-gray-600 focus:outline-none"
                value={formData.slug}
                readOnly // Suggest making it read-only or editable if needed, keeping it editable for flexibility but auto-filled
                onChange={handleInputChange}
              />
            </div>
          </div>
        ...
        <section className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Original Price (if on sale)</label>
              ...
              {formData.whatYouWillMaster.map((item, index) => (
                <div key={index} className="flex items-center gap-2">
                  <input
                    ...
                    placeholder="e.g. Advanced SEO strategies..."
                  />
                ...
                <div className="w-full aspect-video bg-white border border-dashed border-gray-300 rounded-lg flex items-center justify-center overflow-hidden cursor-pointer relative group">
                  {result.imageUrl ? (
                    ...
                    <span className="text-xs text-gray-500 flex flex-col items-center">
                      <Upload className="w-4 h-4 mb-1" /> Upload Image
                    </span>
                  )}
